...
def preprocess_sub_csv_file_and_send2new_directory(raw_csv_file_path, saved_file_path):
...
def send_files_under_subfolder(old_subfolder_path, new_subfolder_path, city_name):
    ...
    for f in os.listdir():
        ...
        if f_ext == '.csv':
            ...
            preprocess_sub_csv_file_and_send2new_directory(csv_path, saved_path)

def find_subcities4preprocess(raw_main_folder_path, new_main_folder_path):
    for city_name in os.listdir(raw_main_folder_path):
        print(city_name)
        send_files_under_subfolder(raw_main_folder_path, new_main_folder_path, city_name)
